"""
DAG Diagnostics System for Airflow/MWAA
...
class DiagnosticQueries:
    ...
    def get_task_duration_trends_query(analysis_days: int) -> str:
        ...
        SELECT 
            ...
            ts.task_id,
            ts.execution_count,
            ROUND(ts.avg_duration, 2) as avg_duration_seconds,
            ROUND(ts.stddev_duration, 2) as stddev_duration_seconds,
            ROUND(ts.min_duration, 2) as min_duration_seconds,
            ROUND(ts.max_duration, 2) as max_duration_seconds,
            ROUND(ts.median_duration, 2) as median_duration_seconds,
            ROUND(ts.p95_duration, 2) as p95_duration_seconds,
            ROUND(rvo.recent_avg, 2) as recent_avg_duration,
            ROUND(rvo.old_avg, 2) as old_avg_duration,
            ROUND(((rvo.recent_avg - rvo.old_avg) / NULLIF(rvo.old_avg, 0)) * 100, 2) as duration_change_percent
        FROM task_stats ts
    ...
    def get_executor_queue_lag_query(analysis_days: int) -> str:
        ...
        SELECT 
            ...
            task_id,
            COUNT(*) as task_count,
            ROUND(AVG(queue_to_start_seconds), 2) as avg_queue_lag_seconds,
            ROUND(MIN(queue_to_start_seconds), 2) as min_queue_lag_seconds,
            ROUND(MAX(queue_to_start_seconds), 2) as max_queue_lag_seconds,
            ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY queue_to_start_seconds), 2) as p95_queue_lag_seconds
        FROM task_timing
        GROUP BY dag_id, task_id
        HAVING COUNT(*) >= 5
    ...
    def get_pool_utilization_query(analysis_days: int) -> str:
        ...
        SELECT 
            ...
            ROUND(AVG(pu.tasks_in_pool), 2) as avg_tasks,
            MAX(pu.tasks_in_pool) as max_tasks,
            ROUND(AVG(pu.running_tasks), 2) as avg_running,
            MAX(pu.running_tasks) as max_running,
            ROUND((AVG(pu.running_tasks) / NULLIF(ps.slots, 0)) * 100, 2) as avg_utilization_percent,
    ...
    def get_execution_time_variance_query(analysis_days: int) -> str:
        ...
        SELECT 
            ...
            ROUND(STDDEV(duration_seconds), 2) as stddev_duration_seconds,
            ROUND(MIN(duration_seconds), 2) as min_duration_seconds,
            ROUND(MAX(duration_seconds), 2) as max_duration_seconds,
            ROUND((STDDEV(duration_seconds) / NULLIF(AVG(duration_seconds), 0)) * 100, 2) as coefficient_of_variation,
            ROUND((MAX(duration_seconds) - MIN(duration_seconds)) / NULLIF(AVG(duration_seconds), 0), 2) as range_ratio
        FROM task_durations
        GROUP BY dag_id, task_id
    ...
    def get_schedule_drift_query(analysis_days: int) -> str:
        ...
        SELECT 
            dag_id,
            COUNT(*) as run_count,
            ROUND(AVG(drift_seconds), 2) as avg_drift_seconds,
            ROUND(MIN(drift_seconds), 2) as min_drift_seconds,
            ROUND(MAX(drift_seconds), 2) as max_drift_seconds,
            ROUND(STDDEV(drift_seconds), 2) as stddev_drift_seconds,
            ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY drift_seconds), 2) as p95_drift_seconds,
    ...
    def get_scheduling_delay_query(analysis_days: int) -> str:
        ...
        SELECT 
            ...
            ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY lag_seconds), 2) as median_lag_seconds,
            ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY lag_seconds), 2) as p95_lag_seconds,
    ...
    def get_capacity_forecasting_query(analysis_days: int) -> str:
        ...
        SELECT 
            ROUND(avg_daily_tasks, 2) as current_avg_daily_tasks,
            ROUND(task_growth_rate * 86400, 4) as tasks_growth_per_day,
            ROUND(avg_daily_dags, 2) as current_avg_daily_dags,
            ROUND(dag_growth_rate * 86400, 4) as dags_growth_per_day,
            ROUND(avg_task_duration, 2) as avg_task_duration_seconds
    ...
    def get_performance_degradation_query(analysis_days: int) -> str:
        ...
        SELECT 
            dag_id,
            task_id,
            ROUND(overall_avg_duration, 2) as avg_duration_seconds,
            ROUND(duration_trend_slope * 86400 * 7, 4) as duration_increase_per_week,
            ROUND(trend_r2, 4) as trend_confidence,
            CASE 
    ...
    def get_anomaly_detection_query(analysis_days: int) -> str:
        ...
        SELECT 
            ...
            ROUND(AVG(duration_seconds), 2) as avg_anomaly_duration,
            ROUND(AVG(mean_duration), 2) as normal_duration
        FROM anomalies
        GROUP BY dag_id, task_id
    ...
    def get_seasonal_patterns_query(analysis_days: int) -> str:
        ...
        SELECT 
            ...
            END as day_name,
            ROUND(avg_executions_per_hour, 2) as avg_executions_per_hour,
            ROUND(stddev_executions, 2) as stddev_executions,
            max_executions,
            min_executions,
            ROUND((stddev_executions / NULLIF(avg_executions_per_hour, 0)) * 100, 2) as coefficient_of_variation
        FROM pattern_stats
    ...
    def get_scheduler_heartbeat_query(analysis_days: int) -> str:
        ...
        WITH heartbeat_gaps AS (
            ...
            FROM scheduler_job
            WHERE latest_heartbeat >= CURRENT_TIMESTAMP - INTERVAL '{analysis_days} days'
        )
        SELECT 
            hostname,
    ...
    @staticmethod
    def get_dag_parsing_times_query() -> str:
        """Query to analyze DAG parsing performance"""
        return """
        SELECT 
            file_path,
            AVG(duration) as avg_parse_duration_seconds,
            MAX(duration) as max_parse_duration_seconds,
            MIN(duration) as min_parse_duration_seconds,
            COUNT(*) as parse_count,
            MAX(last_duration) as last_parse_duration_seconds,
    ...
    @staticmethod
    def get_file_processing_lag_query() -> str:
        """Query to measure file processing lag"""
        return """
        SELECT 
            file_path,
            MAX(timestamp) as last_processed,
            EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - MAX(timestamp))) as seconds_since_last_process,
            COUNT(*) as process_count,
            AVG(duration) as avg_duration
        FROM dag_processing
        GROUP BY file_path
        HAVING MAX(timestamp) < CURRENT_TIMESTAMP - INTERVAL '10 minutes'
    ...
    def get_scheduler_loop_duration_query(analysis_days: int) -> str:
        ...
        SELECT 
            hostname,
            COUNT(*) as loop_count,
            ROUND(AVG(loop_duration), 2) as avg_loop_duration_seconds,
            ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY loop_duration), 2) as median_loop_duration,
            ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY loop_duration), 2) as p95_loop_duration,
            ROUND(MAX(loop_duration), 2) as max_loop_duration,
    ...
    def get_queued_tasks_duration_query(analysis_days: int) -> str:
        ...
        SELECT 
            ...
            task_id,
            COUNT(*) as task_count,
            ROUND(AVG(queue_duration_seconds), 2) as avg_queue_seconds,
            ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY queue_duration_seconds), 2) as median_queue_seconds,
            ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY queue_duration_seconds), 2) as p95_queue_seconds,
            ROUND(MAX(queue_duration_seconds), 2) as max_queue_seconds,
    ...
    def get_error_clustering_query(analysis_days: int) -> str:
        ...
        WITH error_messages AS (
            SELECT 
                dag_id,
                task_id,
                SUBSTRING(error, 1, 200) as error_snippet,
                start_date,
                duration
            FROM task_fail
            WHERE start_date >= CURRENT_DATE - INTERVAL '{analysis_days} days'
        ...
        error_patterns AS (
            ...
            FROM error_messages
            GROUP BY dag_id, task_id, error_snippet
        )
        SELECT 
            dag_id,
            task_id,
    ...
    def get_failure_cascade_query(analysis_days: int) -> str:
        ...
        WITH dag_failures AS (
            ...
            FROM task_fail
            WHERE start_date >= CURRENT_DATE - INTERVAL '{analysis_days} days'
            GROUP BY dag_id, run_id, execution_date
        ),
        cascade_analysis AS (
            SELECT 
                df.dag_id,
                df.run_id,
                df.execution_date,
                df.first_failure_time,
                COUNT(tf.task_id) as total_failed_tasks,
                COUNT(DISTINCT tf.task_id) as unique_failed_tasks,
                MAX(tf.start_date) as last_failure_time,
                EXTRACT(EPOCH FROM (MAX(tf.start_date) - df.first_failure_time)) as cascade_duration_seconds
            FROM dag_failures df
            JOIN task_fail tf ON df.dag_id = tf.dag_id 
                AND df.run_id = tf.run_id 
                AND df.execution_date = tf.execution_date
            GROUP BY df.dag_id, df.run_id, df.execution_date, df.first_failure_time
            HAVING COUNT(DISTINCT tf.task_id) >= 3
        )
        SELECT 
            dag_id,
            COUNT(*) as cascade_count,
            ROUND(AVG(unique_failed_tasks), 2) as avg_tasks_per_cascade,
            MAX(unique_failed_tasks) as max_tasks_in_cascade,
            ROUND(AVG(cascade_duration_seconds), 2) as avg_cascade_duration_seconds
...
def analyze_scheduler_performance(**context):
    ...
    try:
        parsing_query = DiagnosticQueries.get_dag_parsing_times_query()
        parsing_records = postgres_hook.get_records(parsing_query)
        
        if parsing_records:
            print(f"{'File Path':<50} {'Avg (s)':<10} {'Max (s)':<10}")
            print("-" * 75)
            
            slow_parsing = []
            
            for record in parsing_records[:15]:  # Top 15
                file_path = record[0]
                avg_duration = float(record[1]) if record[1] else 0
                max_duration = float(record[2]) if record[2] else 0
                
                file_name = file_path.split('/')[-1] if '/' in file_path else file_path
                if len(file_name) > 47:
                    file_name = file_name[:44] + "..."
                
                print(f"{file_name:<50} {avg_duration:<10.2f} {max_duration:<10.2f}")
                
                scheduler_analysis["parsing_times"].append({
                    "file_path": file_path,
                ...
                if avg_duration > 30:
                    slow_parsing.append(file_name)
            
            if slow_parsing:
                scheduler_analysis["insights"].append(
                    f"⚠️ {len(slow_parsing)} DAG files take >30s to parse - optimize DAG code"
                )
                
    except Exception as e:
        print(f"Error analyzing parsing times: {e}")
        scheduler_analysis["parsing_times"] = {"error": str(e)}

    # 3. File Processing Lag
    print("\n⏱️ FILE PROCESSING LAG")
    print("-" * 40)
    
    try:
        lag_query = DiagnosticQueries.get_file_processing_lag_query()
        lag_records = postgres_hook.get_records(lag_query)
        
        if lag_records:
            print(f"{'File Path':<50} {'Minutes Since Process':<20}")
            print("-" * 75)
            
            stale_files = []
            
            for record in lag_records[:10]:  # Top 10
                file_path = record[0]
                seconds_since = float(record[2]) if record[2] else 0
                ...
                print(f"{file_name:<50} {minutes_since:<20.1f}")
                
                scheduler_analysis["processing_lag"].append({
                    "file_path": file_path,
                    "seconds_since_last_process": seconds_since
                })
            ...
            if stale_files:
                scheduler_analysis["insights"].append(
                    f"⚠️ {len(stale_files)} files not processed in >1 hour - check scheduler"
                )
        else:
            print("✅ All files recently processed")
            
    except Exception as e:
        print(f"Error analyzing processing lag: {e}")
        scheduler_analysis["processing_lag"] = {"error": str(e)}

    # 4. Scheduler Loop Duration
